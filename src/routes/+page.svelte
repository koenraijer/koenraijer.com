<script>
	...
	// SEO
	const ogImage = `https://koenraijer-og.vercel.app/api/og?title=${encodeURIComponent(
		info.name
	)}&description=${encodeURIComponent(info.bio.text)}`;
</script>
...
<!-- Current Status -->
<section class="section md:max-w-5xl h-fit md:mt-12" aria-labelledby="now-heading">
	<h2 id="now-heading" class="text-sm font-[500] text-muted-foreground mb-2">Now</h2>
	<p class="text-sm">
		<span
			>Slowing down, learning, reflecting. Working at a mental health institution. Finishing up on
		</span>
		<A classes="text-sm anchor" href="https://doi.org/10.17605/OSF.IO/CR5F8">research</A> on ambulatory
		assessment in PTSD.
	</p>
</section>
...
<div class="section md:max-w-5xl flex flex-col md:grid md:grid-cols-2 md:gap-x-4 h-auto md:mt-12">
	<!-- Reading section -->
	<section class="mt-0 h-fit" aria-labelledby="reading-heading">
		<h2 id="reading-heading" class="text-sm font-[500] text-muted-foreground mb-2">Reading</h2>
		<div class="flex flex-col gap-y-1" role="list" aria-label="Currently reading">
...
<!-- Writing section -->
<section class="section md:max-w-5xl h-fit" aria-labelledby="writing-heading">
	<h2 id="writing-heading" class="text-sm font-[500] text-muted-foreground mb-2">Writing</h2>
	<Posts posts={data.posts} />
</section>

<!-- Contact section -->
<section class="section md:max-w-5xl h-fit" aria-labelledby="connect-heading">
	...
	<p class="text-sm flex items-center gap-2 flex-wrap">
		<span
			>Reach me at <A me href={'mailto:' + info.email} aria-label={'Send email to ' + info.email}
				>{info.email}</A
			>. Please note: this email is not intended for (former) patient/client matters.</span
		>
	</p>
</section>
