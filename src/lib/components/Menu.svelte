...
<div class="fixed sm:relative z-50" use:clickOutside on:outclick={handleClickOutside}>
    <button
        class="transition-colors decoration-none flex justify-center items-center hover:bg-surface-100-800-token p-2 cursor-pointer duration-[200ms] border border-surface-900-50-token rounded-container bg-surface-50-900-token"
    ...
    >
        ...
        <div
            class="absolute top-full {isOpen ? "visible" : "invisible"} right-0 mt-4 w-fit h-fit transition-colors decoration-none flex flex-col justify-stretch p-2 duration-[200ms] border border-surface-900-50-token rounded-container bg-surface-50-900-token gap-y-2"
        >
            <slot></slot>
        </div>
    </div>
